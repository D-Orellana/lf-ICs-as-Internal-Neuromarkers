...
datadir = 'D:\Brown_Data\Rusty\';
disp('Subsampling RUS120618...')
name_2_save=strcat('RUS120618_',num2str(30000/subsamp_factor));


%% Load events MI_PMd and PMv
file_MI_PMd='RUSRH120618_MI_PMd_TT_KG_TC_NOGO\RUSRH120618_MI_PMd_TT_KG_TC_NOGO001.ns5';
MI_PMd_events=load([datadir 'RUSRH120618_MI_PMd_TT_KG_TC_NOGO\derived\RUSRH120618_MI_PMd_TT_KG_TC_NOGO001_ev_explicit.mat'],'ev_ex');

file_PMv='RUSRH120618_PMv_TT_KG_TC_NOGO\RUSRH120618_PMv_TT_KG_TC_NOGO001.ns5';
PMv_events= load([datadir 'RUSRH120618_PMv_TT_KG_TC_NOGO\derived\RUSRH120618_PMv_TT_KG_TC_NOGO001_ev_explicit.mat'],'ev_ex');
...
temp_data = openNSx_old([datadir file_MI_PMd],'channels' ,1:96,['t:' num2str(1) ':' num2str(samp_fin_MI) ],'sample', 'p:double');
data_MI_PMd= downsample(temp_data.Data',subsamp_factor);
clear vars temp_data last_trial_MI_PMd samp_fin_MI 
    
temp_data= openNSx_old([datadir file_PMv],'channels' ,1:96,['t:' num2str(1) ':' num2str(samp_fin_PMv) ],'sample', 'p:double');
data_PMv=downsample(temp_data.Data',subsamp_factor);
clear vars temp_data last_trial_PMv samp_fin_PMv
...
ix_exe(bad_events,:)=[];        % algo anda mal con este evento
...
% Events NoGo
ix_NoGo(:,1) = find(events_temp.Completion.Completed & events_temp.Stage.ObjectPresent & events_temp.Mode.NoGo);  
ix_NoGo(:,2) = find(events_temp.Completion.Completed & events_temp.Stage.GraspCue & events_temp.Mode.NoGo);  
ix_NoGo(:,3) = find(events_temp.Completion.Completed & events_temp.Stage.GoCue & events_temp.Mode.NoGo);  
events_NoGo=events_temp.timestamps(ix_NoGo(:,:));

% Labels for observated movements
obID_NoGo =  (events_temp.Object.TC(ix_NoGo(:,3))*10) + (events_temp.Object.KG(ix_NoGo(:,3))*20);    %TC=10, KG=20
gripID_NoGo = events_temp.Grip.Power(ix_NoGo(:,3))+events_temp.Grip.Precision(ix_NoGo(:,3))*2+ events_temp.Grip.Key(ix_NoGo(:,3))*3;      % Power=1,   Presicion=2,  key=3
labels_NoGo = [obID_NoGo + gripID_NoGo];                      
...
aux=find(~events_temp.Completion.Completed & events_temp.Stage.GraspCue);
...
Data_M1_PMd.events_ob=events_NoGo;
...
Data_M1_PMd.labels_ob=labels_NoGo;
...
%% Create behavioral marker table and labels for PMv
events_temp=PMv_events.ev_ex;
ix_exe(:,1) = find(events_temp.Completion.Completed & events_temp.Stage.ObjectPresent & events_temp.Mode.Execution);  
ix_exe(:,2) = find(events_temp.Completion.Completed & events_temp.Stage.GraspCue & events_temp.Mode.Execution);  
bad_events=[21 39 40 48 53 54 59 60 70 76 118 119 122 123 125 132 135 137 138];
...
Data_PMv.events_ob=events_NoGo;
...
Data_PMv.labels_ob=labels_NoGo;
