...
fso=30000;
N=100;
load('D:\LFP_spikes_analisys\SPK_121107\SPK_all_121107_250.mat')
load('D:\LFP_spikes_analisys\SPK_121107\W_ICA_SPK121107_250.mat')
load('prefered_chs_SPK_121107.mat')

grip_chs=prefered_chs.grip;
grip_chs=[105 107 111 113 126 137 139 147 148 150];
...
for kch=1:numel(grip_chs)
    kch
       ch_id=grip_chs(kch);
        ...
        for k=1:numel(labels_exe)
            ini=round((locs_grip(k)-0.42)*fso);
            fin=ini + round(0.7*fso);
            int_raster_full(k,:)=raster_vec(ini:fin);
            
            ini=round((events_exe(k,2)-0.1)*fso);
            fin=ini + round(0.7*fso);
