...
function  align_results=align_Grip_RUS(data_folder,file2load, w2load, grip_IC,chs2align)
...
%     xline(events_NM(k,2)*fs,'g','LineWidth',1.5)
% end
...
for kch=1:numel(chs2align)
        ...
        for k=1:numel(labels_exe)
            ini=round((locs_grip(k)-0.46)*fs_raw);
            fin=ini + round(1.5*fs_raw);
            int_raster_full(k,:)=raster_vec(ini:fin);
            
            ini=round((events_exe(k,2)+0.33)*fs_raw);
            fin=ini + round(1.5*fs_raw);
...
%title(strcat('Aligned with ICs Ch= ',num2str(ch_id)),'Color','k');
        ...
        title(strcat('Grip Alingment Improvement, N',num2str(ch_id),' -->',num2str(100*FR_dif_ix(kch))));
