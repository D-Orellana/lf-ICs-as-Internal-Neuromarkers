...
for k=1:numel(labels_exe)
	...
	fin=round((ref+1.1)*fs);               % 700ms after external marker    
    if file2load =='RUS_all_120618_250_rev1.mat'
        fin=round((ref+0.8)*fs);               % 800ms after external marker
    end
	if file2load =='RUS_all_120619_250_rev1.mat'
        fin=round((ref+0.8)*fs);               % 1100ms after external marker
    end
