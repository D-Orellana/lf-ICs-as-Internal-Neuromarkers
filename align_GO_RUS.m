...
function  align_results=align_GO_SPK(data_folder,file2load, w2load, go_IC,chs2align)
...
for kch=1:numel(chs2align)
        ...
        for k=1:numel(labels_exe)
            ini=round((locs_go(k)-0.38)*fs_raw);
            fin=ini + round(1.7*fs_raw);
            int_raster_full(k,:)=raster_vec(ini:fin);
            
            ini=round((events_exe(k,3)-0.0)*fs_raw);
            fin=ini + round(1.7*fs_raw);
...
%title(strcat('Aligned with ICs Ch= ',num2str(ch_id)),'Color','k');
        ...
        title(strcat('Go Alingment Improvement, N',num2str(ch_id),' -->',num2str(100*FR_dif_ix(kch))));
